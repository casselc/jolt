...
(define (jolt-runtime-windows-machine-type? machine)
  ;; Keep this runtime-only boundary local. The historical implementation used
  ;; a target-descriptor helper that is not part of upstream v0.6.4.
...
(define (jolt-runtime-shell-command cmd)
  (if (jolt-runtime-windows-machine-type? (machine-type))
      (let ((f (jolt-runtime-sh-script cmd)))
        ;; cmd.exe strips the first and last quote when a /c command begins
        ;; with a quoted executable. The extra outer pair preserves the two
        ;; independently quoted path arguments after that special parsing.
      ...
      (guard (e (#t (jolt-runtime-delete-script script) (raise e)))
        (call-with-values
          ...
          (lambda (stdin stdout stderr pid)
            (guard (e (#t
                       ;; Preserve the primary close/read exception while
                       ;; releasing every child pipe handle. The outer guard
                       ;; owns deletion of the temporary script.
                       (guard (_ (#t #f)) (close-port stdin))
                       (guard (_ (#t #f)) (close-port stdout))
                       (guard (_ (#t #f)) (close-port stderr))
                       (raise e)))
              (close-port stdin)
