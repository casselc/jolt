...
;; class-provider registry, serialized evaluator, and failure-atomic staging for
;; the dedicated host-registration hooks. Retry-on-miss dispatch and concurrent
;; reader isolation remain deliberately unwired. After the value/collection/error
;; layers (jolt-throw/jolt-ex-info/jolt-hash-map/keyword) and beside
;; class-hierarchy.ss in the host java/ layer; before records.ss so later
;; protocol/extend integration can read class-provider-for.
