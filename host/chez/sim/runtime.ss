...
;; Nothing in this file is loaded by an ordinary release/debug image.
...
(define (ffi-sim-load-library . args)
  ...
  (let ((h (jolt-sim-current-ffi-hook)))
    (if h (jolt-ffi-invoke-native-sim-op h "load-library" args
                                         (lambda () (apply ffi-load-library args)))
        (apply ffi-load-library args))))
(define (ffi-sim-loaded? name)
  (jolt-str-render-one name)
  (let ((h (jolt-sim-current-ffi-hook)))
    (if h (jolt-ffi-invoke-native-sim-op h "loaded?" (list name)
                                         (lambda () (ffi-loaded? name)))
        (ffi-loaded? name))))
(define (ffi-sim-alloc n)
  (ffi-validate-allocation-size n)
  (let ((h (jolt-sim-current-ffi-hook)))
    (if h (jolt-ffi-invoke-native-sim-op h "alloc" (list n)
                                         (lambda () (ffi-alloc n)))
        (ffi-alloc n))))
(define (ffi-sim-free p)
  (ffi-validate-pointer p)
  (let ((h (jolt-sim-current-ffi-hook)))
    (if h (jolt-ffi-invoke-native-sim-op h "free" (list p)
                                         (lambda () (ffi-free p)))
        (ffi-free p))))
(define (ffi-sim-read p ty . off)
  (ffi-validate-read-args p ty off)
  (let ((h (jolt-sim-current-ffi-hook)))
    (if h (jolt-ffi-invoke-native-sim-op h "read" (cons p (cons ty off))
...
(define ffi-sim-write-array
  (case-lambda
    ((p arr)
     ...
     (let ((h (jolt-sim-current-ffi-hook)))
       (if h (jolt-ffi-invoke-native-sim-op h "write-array" (list p arr)
                                            (lambda () (ffi-write-array p arr)))
           (ffi-write-array p arr))))
    ((p arr off n)
     (ffi-validate-write-array-range p arr off n)
     (let ((h (jolt-sim-current-ffi-hook)))
       (if h (jolt-ffi-invoke-native-sim-op h "write-array" (list p arr off n)
                                            (lambda () (ffi-write-array p arr off n)))
           (ffi-write-array p arr off n))))))
(define (ffi-sim-ptr->string p)
  (ffi-validate-pointer p)
  (let ((h (jolt-sim-current-ffi-hook)))
    (if h (jolt-ffi-invoke-native-sim-op h "ptr->string" (list p)
                                         (lambda () (ffi-ptr->string p)))
        (ffi-ptr->string p))))
(define (ffi-sim-string->ptr s)
  (string->utf8 (jolt-str-render-one s))
  (let ((h (jolt-sim-current-ffi-hook)))
    (if h (jolt-ffi-invoke-native-sim-op h "string->ptr" (list s)
                                         (lambda () (ffi-string->ptr s)))
        (ffi-string->ptr s))))

;; Staged signed-byte loans.  A real proceed allocates and locks the same kind
;; of temporary bytevector as the v0.5.13 base primitive.  Cleanup retires first,
;; then copybacks/unlocks exactly once even if the release observer raises.  A
;; modeled borrow never fabricates a real lock, and proceeding its release fails
...
(define (ffi-sim-with-byte-array-pointer-range arr off len f)
  (let* ((v (ffi-byte-array-vector "with-byte-array-pointer" arr))
    ...
    (unless (procedure? f)
      (throw-jvm (quote IllegalArgumentException)
                 "jolt.ffi/with-byte-array-pointer: callback must be a function"))
...
;; the branch-free v0.5.13 base primitive unchanged.
...
;; System/nanoTime and jolt.host/monotonic-nanos both resolve it at call time.
;; The supervisor primitive below calls the saved procedure directly and can
;; therefore drive jolt-sim's drain watchdog even while modeled time is frozen.
(define jolt-sim-supervisor-now-monotonic-nanos now-monotonic-nanos)
(define-record-type jolt-sim-clock-installation
  (fields hook routing? previous mu (mutable last))
  (nongenerative jolt-sim-clock-installation-v1))
...
(define jolt-sim-kw-monotonic-nanos (keyword #f "monotonic-nanos"))
(define jolt-sim-clock-descriptor
  (jolt-hash-map jolt-sim-kw-kind jolt-sim-kw-clock
                 jolt-sim-kw-operation jolt-sim-kw-monotonic-nanos))
(define (jolt-sim-clock-validate! installation n)
  ...
  (with-mutex (jolt-sim-clock-installation-mu installation)
    (let ((last (jolt-sim-clock-installation-last installation)))
      (when (and last (< n last))
        (error 'jolt-sim-clock "monotonic clock moved backward"))
      (jolt-sim-clock-installation-last-set! installation n)))
  n)
(define (jolt-sim-now-monotonic-nanos)
  (let ((installation (jolt-sim-current-clock-installation)))
    (if (not installation)
        (jolt-sim-supervisor-now-monotonic-nanos)
        (let ((owner (get-thread-id)))
            ...
            (jolt-sim-clock-validate!
             ...
             (if (jolt-sim-clock-installation-routing? installation)
                 ...
                 (jolt-invoke (jolt-sim-clock-installation-hook installation)
                              jolt-sim-clock-descriptor))))))))
(set! now-monotonic-nanos jolt-sim-now-monotonic-nanos)
(define (jolt-sim-supervisor-monotonic-nanos)
  (->num (jolt-sim-supervisor-now-monotonic-nanos)))
...
(define jolt-sim-kw-supervisor-monotonic-nanos (keyword #f "supervisor-monotonic-nanos"))
...
(define (jolt-sim-capabilities)
  (jolt-hash-map
   ...
   (jolt-hash-map
    ...
    jolt-sim-kw-operations (jolt-vector jolt-sim-kw-monotonic-nanos)
    jolt-sim-kw-result jolt-sim-kw-exact-integer-nanoseconds
    jolt-sim-kw-nondecreasing? #t
    jolt-sim-kw-supervisor-operation jolt-sim-kw-supervisor-monotonic-nanos
...
(define-record-type jolt-sim-controller-installation
  ...
  (nongenerative jolt-sim-controller-installation-v5))
...
(define (jolt-sim-install-controller! config)
  ...
  (let* ((controllers (jolt-sim-validate-controller-config config))
    ...
    (with-mutex jolt-sim-controller-mu
      ...
      (let* ((ffi* (make-jolt-ffi-sim-hook-installation
                    ffi-wrapper #t jolt-ffi-sim-hook-top))
             (clock* (make-jolt-sim-clock-installation
                      clock-controller #t jolt-sim-clock-top (make-mutex) #f))
             (i (make-jolt-sim-controller-installation
                  future ffi-controller ffi* clock* jolt-sim-controller-top)))
...
(def-var! "jolt.internal.sim" "supervisor-monotonic-nanos"
          jolt-sim-supervisor-monotonic-nanos)
