           ...
           (let ((port (in-stream-port self)))
             (if (null? rest)
                 ...
                 (let* ((buf (car rest))
                        ...
                        (len (if (>= (length rest) 3) (jnum->exact (caddr rest)) (ja-len vec))))
                   (let loop ((i 0))
                     (if (>= i len) (->num i)
                         (let ((b (get-u8 port)))
                           (if (eof-object? b)
                               (if (= i 0) -1 (->num i))
                               (begin (ja-set! vec (+ off i) b) (loop (+ i 1))))))))))))
                   ...
                   (let loop ((i 0))
                     (if (>= i len) (->num i)
                         (let ((c (get-char port)))
                           (if (eof-object? c)
                               (if (= i 0) -1 (->num i))
                               (begin (ja-set! vec (+ off i) c) (loop (+ i 1))))))))))))
    ...
    ((and (htable? output) (jolt-truthy? (jolt-ref-get output (keyword "jolt" "output-stream"))))
     (let ((bv (input-bytes input)))
       (record-method-dispatch output "write"
         ;; via na-byte-array (natives-array.ss), not a hand-built backing — the
         ;; byte-array's storage is that file's business.
         (list->cseq (list (if bv (na-byte-array bv)
                               (input-text input)))))))
