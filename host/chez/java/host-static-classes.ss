...
(define (al-to-array self args)
  (let ((n (al-cnt self)))
    (cond
      ;; Collection.toArray() returns a new Object[], never a Clojure vector.
      ((null? args)
       (make-jolt-array (list->vector (al->list self)) 'object))
      ((null? (cdr args))
       (let ((dst (car args)))
...
(define arraylist-methods
  (list
    ...
    (cons "toArray" (lambda (self . args) (al-to-array self args)))
