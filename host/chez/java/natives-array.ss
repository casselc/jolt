...
;; semantics (a snapshot read-then-write, so an overlapping copy is not
;; corrupted). Validate the whole operation before mutating either array.
;; Primitive array kinds must agree even when they share one Chez backing type
;; (float/double are both flvectors; every other kind is a boxed vector). Jolt
;; models one Object[] kind, so that is the full reference-array compatibility
;; question its value model can represent. Elements ride the existing signed-byte
;; representation (a copied -1 stays -1, never 255).
(define na-vector-copy-proc
  (and (top-level-bound? 'vector-copy!) (top-level-value 'vector-copy!)))
(define na-flvector-copy-proc
  (and (top-level-bound? 'flvector-copy!) (top-level-value 'flvector-copy!)))
;; DIRECTION-SAFE loop: the only overlap case a forward copy corrupts is a
;; backward-overlapping self-copy (same backing, dest past source, dest within
;; the source span); copy that one top-down, everything else bottom-up. This is
;; memmove, not memcpy, behavior, and it is what self-overlap always lands in —
;; distinct jolt-arrays never share a backing, so the bulk path below is only
;; ever taken for non-overlapping ranges.
(define (na-arraycopy-loop! sv s dv d n)
  (if (and (eq? sv dv) (fx>? d s) (fx<? d (fx+ s n)))
      (let loop ((i (fx- n 1)))
        (when (fx>=? i 0)
          (ja-set! dv (fx+ d i) (ja-ref sv (fx+ s i)))
          (loop (fx- i 1))))
      (let loop ((i 0))
        (when (fx<? i n)
          (ja-set! dv (fx+ d i) (ja-ref sv (fx+ s i)))
          (loop (fx+ i 1))))))
;; bulk copy for non-overlapping ranges (distinct backings): a single Chez
;; vector-copy!/flvector-copy! when available, else the direction-safe loop.
(define (na-arraycopy-bulk! sv s dv d n)
  (cond
    ((and (flvector? sv) na-flvector-copy-proc) (na-flvector-copy-proc sv s dv d n) #t)
    ((and (vector? sv) na-vector-copy-proc) (na-vector-copy-proc sv s dv d n) #t)
    (else (na-arraycopy-loop! sv s dv d n) #t)))
(define (na-system-arraycopy src src-off dst dst-off len)
  ...
  (let* ((s (jnum->exact src-off))
    ...
    (unless (fx=? n 0)
      (if (eq? sv dv) (na-arraycopy-loop! sv s dv d n) (na-arraycopy-bulk! sv s dv d n)))
    jolt-nil))

;; System was registered earlier by host-static-methods.ss / host-static-classes.ss.
;; register-class-statics! merges a member into that shared short/FQN table, so
;; both System/arraycopy and java.lang.System/arraycopy resolve identically.
