...
;; Lazily derived membership and simple-name indices. They must be invalidated
;; by the same mutation boundary as the closure/tag caches: providers extend the
;; graph after either index may already have answered a miss.
...
(def-var! "jolt.host" "register-class-supers!"
  (lambda (name supers)
    (let ((supers (seq->list supers)))
      ;; class-provider-stage-operation! is loaded later with the static
