  ...
  (list
    ...
    (cons "slice" (lambda (self)
                    (let* ((src (jolt-array-vec (bb-backing self)))
                           ...
                           (n (- (bb-limit self) p))
                           (dst (na-byte-array n)))
                      (bytevector-copy! src p (jolt-array-vec dst) 0 n)
                    ...
                    (cond
                      ...
                      (else
                       (let* ((dst (car args)) (rest (cdr args)) (dv (jolt-array-vec dst))
                              ...
                              (len (if (and (pair? rest) (pair? (cdr rest)))
                                       (jnum->exact (cadr rest))
                                       (ja-len dv)))
                              (p (bb-pos self)))
                         (bytevector-copy! src p dv off len)
                         (bb-pos! self (+ p len))
                         self))))))))
