...
(define (bld-sh-capture/status cmd)
  (let ((out (bld-probe-temp-path "stdout")) (rc #f) (text ""))
    ...
    ;; The caller removes this file only after the complete identity check
    ;; succeeds.  A failed or crashing child therefore leaves its raw probe
    ;; output available for a one-shot build investigation.
    (vector rc text out)))
...
(define bld-chez-resolved
  (let ((p (if bld-chez-requested
               ...
               (bld-sh-capture
                 "command -v chez || command -v chezscheme || command -v scheme"))))
...
(define (bld-probe-chez)
  (or bld-chez-probe
      (let* ((version-result
             ...
             (probe (vector
                      (vector-ref version-result 0)
                      (bld-last-token (vector-ref version-result 1))
                      (vector-ref machine-result 0)
                      (bld-last-token (vector-ref machine-result 1))
                      (vector-ref version-result 2)
                      (vector-ref machine-result 2)
                      machine-script)))
        (set! bld-chez-probe probe)
        probe)))

(define (bld-probe-evidence probe)
  (string-append
    "\nProbe evidence preserved at: "
    (vector-ref probe 4) ", " (vector-ref probe 5) ", "
    (vector-ref probe 6)))

(define (bld-clear-probe-evidence! probe)
  (for-each
    (lambda (p) (when (file-exists? p) (delete-file p)))
    (list (vector-ref probe 4) (vector-ref probe 5) (vector-ref probe 6))))
      ...
      (unless (zero? version-rc)
        (error 'jolt-build
          (string-append
            ...
            (bld-probe-evidence probe))))
      (unless version
        (error 'jolt-build
          (string-append
            "selected child Chez did not report a parseable version: "
            bld-chez (bld-probe-evidence probe))))
      ...
      (unless (string=? machine bld-machine)
        (error 'jolt-build
          (string-append
            ...
            (bld-probe-evidence probe))))
      (bld-clear-probe-evidence! probe))))
