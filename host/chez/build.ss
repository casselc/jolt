...
(define (build-binary entry-ns out-path mode natives embed-dirs ext-roots direct-link? tree-shake? allow-dynamic library? aspect-config)
  (ei-profile-init!)
...
(define (bld-compile-runtime! src so cache?)
  (let* ((body (read-file-string src))
         (cache (and cache? (bld-runtime-cache-enabled?)
                     (bld-runtime-cache-path body))))
...
;; Optional trailing values are target, target-pack, boot-mode, allow-dynamic,
;; and aspect-config. Older callers may stop at any prefix; absent values retain
;; host/default/empty/nil behavior.
(define (bld-opt-ref opt i)
  (let loop ((o opt) (i i))
    (cond ((or (null? o) (< i 0)) #f)
          ((= i 0) (car o))
          (else (loop (cdr o) (- i 1))))))
(define (bld-opt-str opt i)
  (let ((v (bld-opt-ref opt i)))
    (and v (not (jolt-nil? v)) (jolt-str-render-one v))))
  ...
  (lambda (entry out mode natives embed-dirs ext-roots direct-link? tree-shake? . opt)
    (parameterize ((bld-target (bld-opt-str opt 0)) (bld-target-pack (bld-opt-str opt 1))
      ...
      (build-binary (jolt-str-render-one entry)
                    ...
                    (let ((v (bld-opt-ref opt 4))) (if v v jolt-nil))))
    jolt-nil))
