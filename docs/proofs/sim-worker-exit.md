...
`chiasmus_lint` returned no errors. `chiasmus_verify` returned SAT:
...
`chiasmus_lint` returned no errors. `chiasmus_verify` returned UNSAT with this
core:
...
possibly executing body and satisfy the exit-based restoration guard.
...
- one future retains one controller snapshot and nested futures retain their
  parent task identity; and
- ordinary images contain none of the private overlay.

Run serially with a pinned threaded Chez:
