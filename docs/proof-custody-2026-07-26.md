...
| --- | --- | --- | --- |
| Jolt core proposal fork | `codex/upstream-improvements-6-8`, plus the post-v0.5.7 timed-deref slice | `test/chez/formal/` and the linked invariant documents cover AOT provenance, dependency-cache paths, executor admission, timed-latch deadline result selection, byte-window geometry, completion leases, protocol dispatch, and the `deftype`/`defrecord` boundary. | Chez unit, forced mutex/deadline, dependency, build, and platform gates cited by each invariant document. |
