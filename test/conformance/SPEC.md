...
|------|------|--------------|
| `test/chez/corpus.edn` | **The spec.** ~4960 cases of `{:suite :label :expected :actual :portability}`, `:expected` **sourced from reference JVM Clojure**. | `test/conformance/regen-corpus.clj` |
...
produces. Of ~4870 certifiable rows, **all but ~106 allowlisted rows
...
implements. `profile.edn`'s own `:portable-count` / `:non-portable-count`
header is the current tally — read it there, and re-run the `--profile`
invocation above after adding cases, rather than trusting a number copied here:
...
| `:perf/unbounded` | the case blocks or runs unbounded on the oracle (an undelivered `promise`), so reference Clojure has no opinion on it |
