...
;; emission carries one exact scalar/variadic descriptor/native thunk branch.
(import (chezscheme))
...
(define variadic-form
  (jolt-ce-read
   "(jolt.ffi/__cfn \"jolt_missing_variadic_symbol\" [:int32 :double] :int64 {:varargs-after 1})"))
(define variadic-node (analyze (make-analyze-ctx "user") variadic-form))
(set-sim! #f)
(define ordinary (emit node))
(set-sim! #t)
(define simulated (emit node))
(define simulated-variadic (emit variadic-node))
(set-sim! #f)
...
(ok "fixed sim descriptor carries an explicit absent variadic boundary"
    (contains? simulated "\"int64\" #f #f #f (list a0)"))
(ok "variadic sim descriptor carries the exact boundary"
    (and (contains? simulated-variadic "(__varargs_after 1)")
         (contains? simulated-variadic "\"int64\" #f #f 1 (list a0 a1)")))
