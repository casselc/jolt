...
(defn -main [& _]
  ...
  (let [seen (atom [])
    ...
    (try
      ...
      ;; post-publication :exit acknowledgement with a bounded deadline. The
      ;; runtime's restore-controller! is private and enforces no drain policy of
      ;; its own — jolt-sim (the external supervisor) is the one that must wait
      ;; for this acknowledgement before it may call restore.
      ...
      (check "host monotonic interception"
             (= 424242 (jolt.host/mono-nanos)))
      (check "supervisor clock bypass"
             (not= 424242 (jolt.internal.sim/supervisor-mono-nanos)))
      (finally
