...
;; A fiber park is such an exit: it cleans up, and resume raises a named Jolt
;; IllegalStateException rather than leaking an opaque Scheme condition.
;;
...
(ev "(def c-fill-pointer-error
       (jolt.ffi/__cfn \"jolt_test_fill_bytes_error\"
         [:pointer :uint8 :size_t :int] :int
...
;; Native error slots are read in the foreign-call return path, before the
;; pointer loan's copy-back and unlock cleanup can overwrite them. A caller must
;; capture inside the callback rather than read ffi/errno after the scope exits.
(ok "atomic native-error capture survives output-loan cleanup"
    (jolt-truthy?
      (ev "(let [a (byte-array [1 2])
                  result
                  (jolt.ffi/with-byte-array-pointer
                    a :out
                    (fn [p n] (c-fill-pointer-error p 207 n 73)))]
              (and (= [-1 73] result)
                   (= [-49 -49] (vec a))))")))
