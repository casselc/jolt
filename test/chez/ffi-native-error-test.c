 ...
 * host. An unrecognized target fails sa-foreign-procedure-native-error at
...
/* String results still cross jolt's Scheme/Clojure conversion boundary after
 * Chez has atomically captured the native error value. */
JOLT_NE_EXPORT const char *jolt_ne_string(int code) {
  JOLT_NE_SET(code);
  return "captured";
}

JOLT_NE_EXPORT const char *jolt_ne_null_string(int code) {
  JOLT_NE_SET(code);
  return (const char *)0;
}
