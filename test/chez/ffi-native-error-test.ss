...
;;   * variadic convention and nullable string-result conversion composition.
...
(ev "(def c-string-err
       (jolt.ffi/__cfn \"jolt_ne_string\" [:int] :string
         {:capture-native-error true}))")
(ev "(def c-null-string-err
       (jolt.ffi/__cfn \"jolt_ne_null_string\" [:int] :string
         {:capture-native-error true}))")
(ok "string result conversion preserves the captured native error"
    (evb "(= [\"captured\" 23] (c-string-err 23))"))
(ok "NULL string result conversion preserves the captured native error"
    (evb "(= [nil 24] (c-null-string-err 24))"))
...
(ok "capture rejected for by-value return"
    (raises? (lambda ()
               (ev "(jolt.ffi/__cfn \"jolt_ne_fail\" []
                       [:by-value [:struct [[:x :int32]]]]
                       {:capture-native-error true})"))))
