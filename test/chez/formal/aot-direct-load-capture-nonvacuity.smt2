; Expected result: SAT.
; Non-vacuity control: a capture with no direct load publishes, its selected
; generation executes, and no duplicate effect or route escape occurs.
;
; Observed witness:
;   capture_shape=capture_without_direct_load
;   nested_operation=no_nested_operation
;   candidate_publishable=true, selected_generation_exec=true
;   nested_side_effect_count=0, violation=false
;
...
(assert (! (= capture_shape capture_without_direct_load)
           :named boundary_no_direct_load_capture))
(assert (! (= nested_operation no_nested_operation)
           :named boundary_no_nested_operation))
(assert (! candidate_publishable
           :named nonvacuous_candidate_publishes))
(assert (! selected_generation_exec
           :named nonvacuous_selected_generation_executes))
(assert (! (= nested_side_effect_count 0)
           :named no_nested_effect_replay))
