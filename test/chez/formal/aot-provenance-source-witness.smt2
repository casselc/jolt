...
; This uses the same violation schema as the general v3 model, constrains the
; producer contexts equal, and requires an edit without reload. The witness must
; therefore arise from loaded-source versus current-disk provenance.
;
; Observed witness: M loaded=s0, M disk/manifest=s1, both producer contexts are
; unchecked, cache_exec=true, artifact_semantics=23, current_semantics=31.
;
...
; Source-mismatch SAT control. This is the same unified schema/query as the
; production model, with two extra witness-shaping assertions: producer contexts
; are equal and an edit-without-reload occurred. SAT must therefore arise from
; retained loaded source versus current disk, not compiler context.
