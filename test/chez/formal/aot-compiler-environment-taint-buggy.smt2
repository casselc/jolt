; Expected result: SAT.
; Buggy environment policy: compile-relevant project invalidation and dirty
; consumed baked observations are neither permanently tainted nor rechecked, so
; a baked nonmacro promotion plus project alias retarget is accepted as a hit.
;
; Observed witness:
;   macro_at_final_guard=macro_state
;   alias_at_final_guard=target_p2
;   registry_at_final_guard=registry_rev1
;   project_generation_invalidated=true
;   baked_consumed_cell_dirty=true
;   consumed_global_registry_dirty=true
;   global_permanent_compile_taint=false
;   candidate_accepted=true, violation=true
;
...
(assert (! (= taint_before_operation false)
           :named witness_process_initially_untainted))
...
; These are the live in-memory values at the final hit/publication decision.
...
(assert (! (= permanent_taint_tracks_unsafe_change false)
           :named buggy_permanent_taint_tracking_disabled))
(declare-const final_publication_live_recheck Bool)
(assert (! (= final_publication_live_recheck false)
           :named buggy_final_publication_recheck_disabled))
...
(assert (! (= cell_origin baked_cell) :named witness_baked_cell))
(assert (! (= mapping_origin project_mapping)
           :named witness_project_mapping))
(assert (! (= cell_consumption consumed)
           :named witness_baked_cell_consumed))
(assert (! (= mapping_consumption consumed)
           :named witness_mapping_consumed))
(assert (! (= registry_consumption consumed)
           :named witness_registry_lookup_consumed))
(assert (! (= root_binding_mode ordinary_late_bound_root)
           :named witness_root_late_bound))
(assert (! (= cache_decision cache_hit_decision)
           :named witness_cache_hit))
(assert (! (= mutation_window change_observed_by_taint)
           :named witness_change_observed_by_taint))
(assert (! (= mutation_actor untrusted_actor)
           :named witness_untrusted_mutation))
(assert (! (= operation_scope outside_loader_scope)
           :named witness_outside_loader_scope))
(assert (! (= safety_strategy scoped_invalidation_strategy)
           :named witness_scoped_strategy))
(assert (! (= namespace_start preexisting_namespace)
           :named witness_preexisting_namespace))
(assert (! (= root_at_final_guard root_rev0)
           :named witness_root_unchanged))
(assert (! (= defined_at_final_guard defined_state)
           :named witness_defined_unchanged))
(assert (! (= macro_at_final_guard macro_state)
           :named witness_baked_nonmacro_promoted))
(assert (! (= meta_at_final_guard meta_rev0)
           :named witness_meta_unchanged))
(assert (! (= alias_at_final_guard target_p2)
           :named witness_alias_retargeted_p1_to_p2))
(assert (! (= refer_at_final_guard target_p1)
           :named witness_refer_unchanged))
(assert (! (= registry_at_final_guard registry_rev1)
           :named witness_global_registry_changed))
(assert (! baked_nonmacro_promoted :named witness_promotion_confirmed))
(assert (! alias_retargeted_p1_to_p2 :named witness_retarget_confirmed))
