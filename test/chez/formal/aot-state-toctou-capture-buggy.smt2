...
; Known-buggy control for capture-time state: per-form revalidation is disabled.
; The deterministic witness changes both compiler context and normalized live
; data-reader identity before form 2, yet the artifact publishes.
;
; Observed witness:
;   capture_context=ctx0, capture_readers=readers0
;   form_1_context=ctx0, form_1_readers=readers0
;   form_2_context=ctx1, form_2_readers=readers1
;   per_form_revalidation=false
;   capture_state_coherent=false, capture_publish=true
;   warm_exec=true, capture_violation=true, warm_violation=false
;   violation=true
;
...
; Six ordered abstract stages in one sequential trace.
...
(assert (! (= per_form_revalidation false)
           :named buggy_missing_per_form_revalidation))
(assert (! (= embedded_execution_state_guard true)
           :named corrected_first_form_guard_enabled))
...
; Witness shaping isolates the capture/publication race.
...
(assert (! (= form_2_context ctx1) :named witness_form_2_context_mutated))
(assert (! (= form_2_readers readers1) :named witness_form_2_readers_mutated))
...
(assert (! (= first_form_context ctx0) :named witness_execution_context))
(assert (! (= first_form_readers readers0) :named witness_execution_readers))
