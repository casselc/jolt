; Expected result: SAT.
; Buggy artifact boundary: root lookup selects a sealed route, a hook mutates one
; provider generation, and an omitted in-memory token guard still commits the
; selection and starts user forms.
;
; Observed witness:
;   mutation_timing=mutation_before_guard
;   mutation_target=provider_target
;   provider_token_after_hook=gen1
;   selection_committed=true, first_user_form_exec=true
;   pre_guard_route_mismatch=true, violation=true
;
...
(assert (! (= artifact_guard_rechecks_all_tokens false)
           :named buggy_artifact_guard_disabled))
...
(assert (! (= mutation_timing mutation_before_guard)
           :named witness_mutation_before_guard))
(assert (! (= mutation_target provider_target)
           :named witness_provider_mutated))
