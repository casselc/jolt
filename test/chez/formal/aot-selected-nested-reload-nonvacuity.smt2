; Expected result: SAT.
; Non-vacuity control: with no nested reload, the selected generation continues
; normal execution without provider mutation or retry.
;
; Observed witness:
;   reload_intent=no_nested_reload
;   provider_mutated=false
;   selected_execution_continues=true
;   retry_started=false, violation=false
;
...
(assert (! (= fail_before_provider_mutation true)
           :named corrected_fail_before_mutation_enabled))
(declare-const retry_after_selected_reload_failure Bool)
(assert (! (= retry_after_selected_reload_failure false)
           :named corrected_post_user_retry_disabled))
...
(assert (! (= reload_intent no_nested_reload)
           :named boundary_no_nested_reload))
(assert (! selected_execution_continues
           :named nonvacuous_selected_execution))
(assert (! (not provider_mutated)
           :named provider_remains_sealed))
(assert (! (not retry_started)
           :named no_retry))
