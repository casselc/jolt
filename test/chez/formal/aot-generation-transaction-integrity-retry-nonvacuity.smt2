; Expected result: SAT.
; Non-vacuity control: an integrity failure occurs before selection or user
; forms, a second bounded attempt is allowed, and user forms are entered exactly
; once. This is not a violating SAT control.
;
; Observed witness:
;   first_attempt_outcome=integrity_failure
;   first_selection_reached=false, first_user_forms_reached=false
...
(assert (! (= first_attempt_outcome integrity_failure)
           :named boundary_integrity_failure))
