; Expected result: SAT.
; Boundary/non-vacuity control for the corrected whole-image contract. A fresh
; compiler process publishes and selects coherent build1, the bounded dynamic
; compile input is declared in the complete graph/input witness, and user forms
; execute without a violation. Independently chosen namespace tokens are set to
; build0 to show that atomic whole-image selection ignores them.
;
; The completeness of the represented graph/input witness is assumed, not
; established by this model. This is not a violating SAT control.
;
; Expected witness:
;   selected_build_token=build1
;   loaded_image_token=build1
;   effective_a_token=build1
;   effective_b_token=build1
;   effective_a_revision=rev1
...
(assert (! (= selected_build_token build1)
           :named boundary_selected_build1))
(assert (! (= loaded_image_token build1)
           :named boundary_loaded_image_build1))
(assert (! (= loaded_image_graph_witness graph1)
           :named boundary_loaded_graph1))
(assert (! (= loaded_image_input_witness inputs1)
           :named boundary_loaded_inputs1))
(assert (! (= loaded_image_bytes bytes1)
           :named boundary_loaded_bytes1))
(assert (! (= loaded_image_a_revision rev1)
           :named boundary_loaded_a_rev1))
(assert (! (= loaded_image_b_revision rev1)
           :named boundary_loaded_b_rev1))
(assert (! (= independently_selected_a_token build0)
           :named ignored_independent_a_build0))
(assert (! (= independently_selected_b_token build0)
           :named ignored_independent_b_build0))
(assert (! (= dynamic_compile_input dynamic_declared)
           :named boundary_dynamic_input_declared))
(assert (! (= fresh_compiler_process true)
           :named boundary_fresh_process))
(assert (! user_forms_exec :named nonvacuous_user_form_execution))
(assert (! (not violation) :named coherent_whole_image_execution))
