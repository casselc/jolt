...
(define expected-capabilities
  (ev "{:abi-version 6
        ...
        :clock-interception {:descriptor-version 1
                             :operations [:monotonic-nanos]
                             :result :exact-integer-nanoseconds
                             :nondecreasing? true
                             :supervisor-operation :supervisor-monotonic-nanos
                             :proceed-routing {:controller-arity 2 :proceed-arity 0
                                               :single-use true :dynamic-extent true
                                               :owner-thread true :lifo true}}}"))
...
(ok "outer unified install owns all three states"
    (and (eq? (jolt-sim-current-future-controller) outer-future)
         (= 111 (ffi-sim-alloc 1)) (= 1000 (now-monotonic-nanos))))
...
(ok "inner unified install replaces all three states"
    (and (eq? (jolt-sim-current-future-controller) inner-future)
         (= 222 (ffi-sim-alloc 1)) (= 2000 (now-monotonic-nanos))))
...
(ok "one install controls System/nanoTime" (= 444 (now-monotonic-nanos)))
...
(ok "supervisor monotonic bypass remains native"
    (not (= 444 (jnum->exact (jolt-sim-supervisor-monotonic-nanos)))))
...
(ok "modeled clock accepts exact nondecreasing nanoseconds" (= 100 (now-monotonic-nanos)))
(ok "modeled clock accepts an equal nanosecond value" (= 100 (now-monotonic-nanos)))
(ok "modeled clock rejects a backward value" (raises? now-monotonic-nanos))
(jolt-sim-restore-controller! clock-token)
(define invalid-clock-token (install native-ffi (lambda (_ __) (kw "not-nanos"))))
(ok "modeled clock rejects a non-integer result" (raises? now-monotonic-nanos))
...
(ok "read-array! destination kind fails before handler"
...
(ok "release observer exception propagates after forced cleanup"
    (raises? (lambda ()
               (ffi-sim-with-byte-array-pointer
                observer-ba (lambda (p n) (foreign-set! 'unsigned-8 p 0 254))))))
...
;; A second ordinary loan of the same array proves the prior temporary unlocked.
(ffi-with-byte-array-pointer observer-ba (lambda (p n) jolt-nil))
(ok "observer exception cleanup left no staged loan lock" #t)
