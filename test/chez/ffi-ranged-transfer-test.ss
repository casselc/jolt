...
;; One side of a transfer is always foreign memory and the other a Scheme-held
;; array, so source and destination cannot alias; the sentinel checks pin that a
;; rejected call and a copy each touch exactly the named range. The existing
;; 2-arg whole-array forms stay exercised for compatibility.
    ...
    (jolt-truthy?
      (ev "(let [dest (byte-array [9 9 9 9])]
              (try
                ...
                false
                (catch IndexOutOfBoundsException _ (= [9 9 9 9] (vec dest)))))")))
(ok "read-array! rejects an overflow-shaped offset before mutating dest"
    (jolt-truthy?
      (ev "(let [dest (byte-array [9 9 9 9])]
              (try
                (jolt.ffi/read-array! jolt.ffi/null 1 dest 9223372036854775807)
                false
                (catch IndexOutOfBoundsException _ (= [9 9 9 9] (vec dest)))))")))
    ...
    (jolt-truthy?
      (ev "(let [src (byte-array [1 2 3 4 5])]
              (try
                ...
                false
                (catch IndexOutOfBoundsException _ (= [1 2 3 4 5] (vec src)))))")))
(ok "write-array ranged rejects an overflow-shaped offset before native access"
    (jolt-truthy?
      (ev "(let [src (byte-array [1 2 3 4 5])]
              (try
                (jolt.ffi/write-array jolt.ffi/null src 9223372036854775807 1)
                false
                (catch IndexOutOfBoundsException _ (= [1 2 3 4 5] (vec src)))))")))
