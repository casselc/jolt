...
# (bench/pagecache.py, posix_fadvise — no root) and reports that number too.
#
...
if [ -n "${COLD:-}" ]; then
  ...
  # usable base; $root is.
  pc="$root/bench/pagecache.py"
  if ! command -v python3 >/dev/null 2>&1 || [ ! -f "$pc" ]; then
    echo
    echo "COLD=1 needs python3 and $pc (posix_fadvise + mincore) — skipping."
    exit 0
  fi
  ...
  cold_ms() {
    first=""; best=""; i=0
    while [ "$i" -lt "$REPS" ]; do
      python3 "$pc" evict "$jolt" >/dev/null 2>&1 || true
      t0=$(perl -MTime::HiRes=time -e 'printf "%d", time()*1000')
