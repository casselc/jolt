...
# (* 1000000 (currentTimeMillis)) -- that is the gate, not decoration.
monotonic:
	@"$${JOLT_BIN:-bin/jolt}" run test/chez/monotonic_clock_test.clj
...
# A forced mutex-reacquisition schedule for the timed-latch deadline boundary.
timedderef:
	@$(CHEZ) --script test/chez/timed-deref-deadline-test.ss
...
ffistress:
	@"$${JOLT_BIN:-bin/jolt}" run test/chez/concurrent_ffi_stress.clj

executorprobe:
	@"$${JOLT_BIN:-bin/jolt}" run test/chez/executor_fidelity_probe.clj
