(ns jolt.ffi
  ...
  result. errno remains the compatibility helper for code that must read the
  current thread's error slot after an existing scalar binding. foreign-callable
...
;; returns [native-result error-code] (result first), with the error slot read in
;; the foreign-call return path. The analyzer validates the literal map and
;; rejects capture on :void.
