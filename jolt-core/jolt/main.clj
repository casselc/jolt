...
(defn- project-basename
  "The final non-root path segment, accepting either native separator."
  [pdir entry]
  (let [segments (remove str/blank?
                         (str/split (str/replace pdir "\\" "/") #"/"))
        segment (last segments)
        drive-only? (and (= 2 (count segment)) (= \: (nth segment 1)))]
